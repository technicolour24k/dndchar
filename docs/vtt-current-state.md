...
**Verification: automated only.** `svelte-check` 0/0, deterministic roll-parity test, dev server boots + endpoint registered/auth-gated. **The live GM+player two-browser click-through is still outstanding** - deliberately NOT marked verified, per this spec's whole reason for existing. Phase 3 Section 2a's "identical to weapon attacks, already verified" pointer is left untouched until that live check passes (spec Section 3).
