...
src/lib/server/services/rollLog.ts       logRoll/listRollLogEntries/getCharacterVttSessionId (roll_log_entries, VTT-room-scoped, not encounter-scoped)  - NEW, Phase 4 Section 1
src/lib/server/services/gameSessions.ts  createGameSession/endGameSession/joinGameSession/listGameSessionsForUser/logSessionNote/listSessionNotes (game_sessions + game_session_notes, user-attributed, not character-attributed)  - NEW, Phase 5 Section 0
src/lib/server/db/migrations/024_combat_log.sql   combat_log_entries table  - NEW, Phase 4 Section 0
src/lib/server/db/migrations/025_roll_log.sql     characters.active_vtt_session_id column + roll_log_entries table  - NEW, Phase 4 Section 1
...
src/routes/characters/[id]/encounters/[encounterId]/   Encounter detail view + "Download as PDF" (window.print() + @media print) - Phase 4 Section 3, now linked from the Activity Log modal's Combat tab rather than its own button (Phase 5 Section 1)
src/routes/sessions/                     "My Sessions" history list + per-session detail/post page (Session Notes) - global nav item, not per-character  - NEW, Phase 5 Section 0
