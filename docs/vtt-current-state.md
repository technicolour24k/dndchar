...
- ~~Character-sheet pull-through is a one-time snapshot~~ - **superseded**: a 30-second polling resync was added (see changelog below) covering reference data (AC, saves, actions, prepared spells, base speed, vision, max HP). `hp` (current) and `spellSlots` (current counts) are deliberately excluded from the poll - they're VTT-session-authoritative once pulled, so a poll tick can't silently overwrite in-progress combat damage or spent slots with the sheet's at-rest values. A real push-based system (sheet save → VTT) is still the longer-term direction once there's an actual realtime layer to hang it on - see the Phase 2 Follow-Up Priorities below.
